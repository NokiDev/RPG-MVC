...
void PlayerEventsHandler::update(sf::Time deltaTime) {
    ...
    if (owner->getBoss()->isKeyPressed(sf::Keyboard::Space)) {
        if (timer >= player->getAttackSpeed()) {
            ...
            ingameController->instantiate(laser, transform->position +
                                                 sf::Vector2f(owner->getSize().x, owner->getSize().y / 4),
                                          sf::Vector2i(1, 0));
            ingameController->instantiate(laser2, transform->position + sf::Vector2f(owner->getSize().x,
                                                                                     owner->getSize().y / 4 +
                                                                                     owner->getSize().y / 2),
                                          sf::Vector2i(1, 0));
        }
